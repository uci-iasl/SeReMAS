import cv2
import numpy as np
import time

# Pretrained classes in the model
classNames = {0: 'background',
              1: 'person', 2: 'bicycle', 3: 'car', 4: 'motorcycle', 5: 'airplane', 6: 'bus',
              7: 'train', 8: 'truck', 9: 'boat', 10: 'traffic light', 11: 'fire hydrant',
              13: 'stop sign', 14: 'parking meter', 15: 'bench', 16: 'bird', 17: 'cat',
              18: 'dog', 19: 'horse', 20: 'sheep', 21: 'cow', 22: 'elephant', 23: 'bear',
              24: 'zebra', 25: 'giraffe', 27: 'backpack', 28: 'umbrella', 31: 'handbag',
              32: 'tie', 33: 'suitcase', 34: 'frisbee', 35: 'skis', 36: 'snowboard',
              37: 'sports ball', 38: 'kite', 39: 'baseball bat', 40: 'baseball glove',
              41: 'skateboard', 42: 'surfboard', 43: 'tennis racket', 44: 'bottle',
              46: 'wine glass', 47: 'cup', 48: 'fork', 49: 'knife', 50: 'spoon',
              51: 'bowl', 52: 'banana', 53: 'apple', 54: 'sandwich', 55: 'orange',
              56: 'broccoli', 57: 'carrot', 58: 'hot dog', 59: 'pizza', 60: 'donut',
              61: 'cake', 62: 'chair', 63: 'couch', 64: 'potted plant', 65: 'bed',
              67: 'dining table', 70: 'toilet', 72: 'tv', 73: 'laptop', 74: 'mouse',
              75: 'remote', 76: 'keyboard', 77: 'cell phone', 78: 'microwave', 79: 'oven',
              80: 'toaster', 81: 'sink', 82: 'refrigerator', 84: 'book', 85: 'clock',
              86: 'vase', 87: 'scissors', 88: 'teddy bear', 89: 'hair drier', 90: 'toothbrush'}


def id_class_name(class_id, classes):
    for key, value in classes.items():
        if class_id == key:
            return value

class Model:
  def __init__(self):
    self.model = self.load_model()
    self.labels = self.load_labels()

  def predict(self, images, target=None):
    if isinstance(images, str):
      images = cv2.imread(images)
    image_height, image_width, _ = images.shape
    self.model.setInput(cv2.dnn.blobFromImage(images, size=(300, 300), swapRB=True))
    output = model.forward()
    if target is None:
      return output
    ret = []
    for detection in output[0, 0, :, :]:
      confidence = detection[2]
      if confidence > .5:
          class_id = detection[1]
          if self.labels(class_id) == target:
            class_name=id_class_name(class_id,classNames)
            print(str(str(class_id) + " " + str(detection[2])  + " " + class_name))
            # box_x = detection[3] * image_width
            # box_y = detection[4] * image_height
            # box_width = detection[5] * image_width
            # box_height = detection[6] * image_height
            x1, x2, y1, y2 = [((detection[3], detection[4], detection[5], detection[6]), confidence)]
            # cv2.rectangle(image, (int(box_x), int(box_y)), (int(box_width), int(box_height)), (23, 230, 210), thickness=1)
            # cv2.putText(image,class_name ,(int(box_x), int(box_y+.05*image_height)),cv2.FONT_HERSHEY_SIMPLEX,(.005*image_width),(0, 0, 255))


  def load_model(self, model_name=None):
    # Loading model
    if model_name is not None:
      if model_name != "ssd_mobilenet_v2_coco_2018_03_29":
        raise ValueError("Only mobilenet v2 available in this library")
    return cv2.dnn.readNetFromTensorflow('models/frozen_inference_graph.pb', 'models/ssd_mobilenet_v2_coco_2018_03_29.pbtxt')

  def download(self, model_name=""):
    pass

  def load_image_into_numpy_array(self, image, expand=True):
    pass

  def load_image_into_numpy_array_cv(self, image, expand=True):
    pass

  def load_labels(self, path_to_labels=''):
    return classNames



if __name__ == "__main__":
  t = []
  try:
    c = cv2.VideoCapture(0)
    m = Model()
    m.predict("image.jpeg")
    exit()
  except:
    pass

  #   cv2.namedWindow("Test")
  #   while True:
  #     t.append(time.time())
  #     ret, frame = c.read()
  #     if ret:
  #       bbox = m.predict(frame, 'person')
  #       if bbox:
  #         size = frame.shape
  #         y1, x1, y2, x2 = bbox[0][0]
  #         x1 = int(float(x1)*float(size[1]))
  #         x2 = int(float(x2)*float(size[1]))
  #         y1 = int(float(y1)*float(size[0]))
  #         y2 = int(float(y2)*float(size[0]))
  #         frame = cv2.rectangle(frame, (x1,y1), (x2,y2), (0, 255, 0), 3)
  #       cv2.imshow("test", frame)
  #       k = cv2.waitKey(1)
  #       if k%256==27:
  #         break
  # finally:
    # print("AVERAGE delay: {} s".format(np.mean([t[i+1]-t[i] for i in range(3, len(t)-1)])))